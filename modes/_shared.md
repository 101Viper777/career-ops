...
1. **Cover letter:** If the form has an option to attach or write a cover letter, ALWAYS include one. Generate PDF with the same visual design as the CV. Content: JD quotes mapped to proof points, links to relevant case studies. 1 page max.
2. Read cv.md and article-digest.md (if exists) before evaluating any offer
...
6b. Keep prompt and mode markdown files in English for consistency, unless explicitly translated on purpose.
...
8c. Application answers and cover letters must sound like a real person, not a template: write in first person, use natural contractions when appropriate, avoid em dashes completely, avoid generic filler, and anchor each answer to one concrete example or motivation.
